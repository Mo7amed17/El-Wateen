...
const OldAccount = () => {
    ...
    useEffect(() => {
            ...
            CallTime.style.display="flex"
                ...
                Inputs.forEach((input)=>{
                    ...
                    input.style.backgroundColor="white"
                        ...
                        Inputs[5].checked=PatientData.alerts
        ...
        if(Object.keys(PatientData).length===0 && secureLocalStorage.getItem("PatientData")!==null){
                setPatientData(secureLocalStorage.getItem("PatientData"))
        }
    }, [PatientData]);

    return (
        <div className="OldAccount">
            <NewAccount ActiveForm={2} values={PatientData}/>
                <div className="MobileLogin">
                    ...
                    <button className="MobileLoginButton" onClick={(e)=>{
                        if(PhoneNumber.length===11 && PhoneNumber.startsWith("01")){
                            ...
                            fetch(`https://el-wateen.mo7amed17.repl.co/Patients?q=${PhoneNumber}`)
                            .then((res)=>res.json())
                            .then((data)=>{
                                if((data.length)===1){
                                    setPatientData(data[0])
                                    SuccessNotification("تم تسجيل الدخول بنجاح",1500)
                                    secureLocalStorage.setItem("LoginPatientAccount","true")
                                    secureLocalStorage.setItem("PatientData",data[0])
                                    secureLocalStorage.setItem("PatientId",data[0].id)
                                    let ActiveH4=document.querySelectorAll(".Top h4")
                                    ActiveH4[1].textContent="حســـابـي"
                                    setTimeout(() => {
                                        MobileLoginBackground.style.display="none"
